...
function setupUI() {
    ...
    if (titleApi) titleApi.textContent = settings.name || "Piantech API";
    if (descApi) descApi.textContent = settings.description || "Interactive API documentation with real-time testing";
    if (footer) footer.textContent = `© ${new Date().getFullYear()} ${settings.creator || "Piantech"} - ${settings.name || "Piantech API"}`;
